...
public class LowerTest extends TestCase {
  public void testValidateParameters() {
    ...
    // Verify that Lower does not accept 0 parameters.
    try {
      lower.validateParameters(Lists.<ValueType>newArrayList());
      fail();
    } catch (InvalidQueryException e) {
      // Do nothing - this is the expected behavior.
    }

    // Verify that Lower does not accept more than 1 parameter.
    try {
      lower.validateParameters(Lists.newArrayList(ValueType.TEXT,
                                                  ValueType.TEXT));
      fail();
    } catch (InvalidQueryException e) {
      // Do nothing - this is the expected behavior.
    ...
    // Verify that Lower does not accept a non-Text parameter.
    try {
      lower.validateParameters(Lists.newArrayList(ValueType.DATE));
      fail();
    } catch (InvalidQueryException e) {
      // Do nothing - this is the expected behavior.
    }

    // Verify that Lower accepts 1 TEXT parameter.
    try {
      lower.validateParameters(Lists.newArrayList(ValueType.TEXT));
    } catch (InvalidQueryException e) {
      fail();
